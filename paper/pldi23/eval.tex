...
\begin{itemize}
 ...
 in JavaScript implementations are detected by synthesized conformance tests?
  ...
  \item \textbf{RQ4 (Comparison with Test262):} \inblue{Can conformance tests
...
found \inred{107} distinct conformance bugs, and reported them to
...
\begin{figure}
  ...
  \begin{subfigure}{0.49\textwidth}
    ...
    \caption{The changes by time.}
  \end{subfigure}
  \begin{subfigure}{0.49\textwidth}
    \includegraphics[width=\textwidth]{img/test262-venn}
    \caption{The venn diagrams.}
  \end{subfigure}
  \caption{
    The changes by time and venn diagrams of covered $k$-FS-TPs and
    $k$-FCPS-TPs for the synthesized conformance tests (\sname{syn-test}) via
    $\tool$ and conformance tests in Test262 (\sname{test262}).
  }
  \label{fig:test262}
\end{figure}
...
mechanized specification extracted from $\esmeta$.
%
Thus, we need to filter out conformance tests in Test262 that utilize language
features not supported in the extracted mechanized specification for a fair
comparison.
%
We follow the methodology usually used in prior work~\cite{kjs, jiset, javert}
to remove inapplicable tests in Test262.
%
Then, we measured the coverage of 23,910 applicable Test262 conformance tests
with five different $k$-FS and $k$-FCPS node-or-branch coverage criteria.
%
Figure~\ref{fig:test262} shows (a) the changes by time and (b) venn diagrams of
covered $k$-FS-TPs and $k$-FCPS-TPs for the synthesized conformance tests
(\sname{syn-test}) via $\tool$ and conformance tests in Test262
(\sname{test262}).
%
...
%
On the other hand, number of $k$-FS-TRs only covered by synthesized tests
increase when using higher $k$: 28.7K (19.0\%) 1-FS-TRs and 1.65M (80.8\%)
2-FS-TRs.
%
In addition, the number of 1-FCPS-TRs (or 2-FCPS-TRs) only covered by
synthesized tests is higher than the number of 1-FS-TRs (or 2-FS-TRs): increse
when using higher $k$: 80.0K (21.6\%) 1-FCPS-TRs and 3.26M (82.0\%) 2-FCPS-TRs.
%
We believe that this is why $\tool$ successfully detected diverse new bugs in
existing JavaScript implementations, even though they are heavily tested
using Test262.

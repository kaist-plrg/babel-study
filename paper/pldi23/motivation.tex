...
\jscode{Number} API feature \jscode{Number} ($\numfeat$).
...
sides of \jscode{1 + 2} are both Number values rather than BigInt values.
%
...
\begin{equation}\label{equ:pool}
  \cdots \qquad \jscode{2n + 1;} \qquad \cdots
\end{equation}
%
Then, the node $17$ is covered by the program \jscode{2n + 1;} because it has
different types of numeric values, a BigInt \jscode{2n} and a Number \jscode{1},
as the left- and right-hand sides of the addition operator (\scode{+}).
%
It means that another similar program \jscode{2n - 1;} might not be added to the
program pool because the test requirement for the node $17$ is already covered
by \jscode{2n + 1;}.
%
...
%
Now, assume that the current program pool is same with the pool in
(\ref{equ:pool}).
%
...
program pool because the test requirement for the node $21$ is already covered
by \jscode{2n + 1;}.
%
However, the conformance test inferred from \jscode{1 + 2n;} is also a
meaningful test case because it checks the edge case when the right-hand side of
the addition operator (\scode{+}) is a BigInt value.
